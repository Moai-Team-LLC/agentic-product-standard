...
| One abstraction point; provider = config (Model & provider M2) | `bifrost/` | pinned Bifrost image + GitOps `config.json`; `agw bifrost-config` regenerates providers from the routing policy |
| Eval-sourced model selection, source recorded (Model & provider M2) | `src/routing/` | policy built from an AgenticPerformance eval export; `eval_run_id` + `synced_at` stored; ranking → Bifrost `fallbacks`; changing the eval run changes routing (tested) |
| One client credential; upstream keys vaulted (Model & provider M2) | `src/vault/` | `sk-agw-*` stored as sha256 → per-tenant Bifrost virtual key, AES-256-GCM at rest, rotation without client change; leak-free tests |
| Tenant-scoped everything + leakage test in CI (Model & provider M3) | `tests/isolation.test.ts` | cache, budgets, vault, routing, ledger, evidence proven tenant-isolated |
