...
> **Reference implementation (paved road):** for running *many* long-lived agents as deployed infrastructure — durable backlog, coordinated scheduling, a bounded runner, fleet health — the family's Runtime & fleet operations surface is **[AgenticOps](https://github.com/Moai-Team-LLC/AgenticOps)** (`bun add github:Moai-Team-LLC/AgenticOps`), via the [`reference-stack`](../reference-stack/SKILL.md) skill. Recommended default, not a requirement — the options below stand on their own.
