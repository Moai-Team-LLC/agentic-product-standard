...
> **Reference implementation (paved road):** **AgenticPerformance (APL)** — the family's Evals & observability surface — ships golden-set evals with a CI regression gate, an error taxonomy, and a governed improvement loop over OpenTelemetry, engine-agnostic. *(private beta — opening soon.)* Set it up via the [`reference-stack`](../reference-stack/SKILL.md) skill, or bring your own (LangSmith / Langfuse / Braintrust / Phoenix). The standard stays neutral — this is the recommended default, not a requirement.
