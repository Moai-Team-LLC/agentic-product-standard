...
> Companion to [`STANDARD.md`](../../STANDARD.md) — the product-level canon. This is the **single-agent** track; the [`agent-builder`](SKILL.md) Claude Code skill operationalizes it (this doc is bundled with that skill), and copy-paste artifacts live in [`templates/`](templates/). Evidence for the load-bearing claims is in the **Evidence & Sources** appendix at the end.
